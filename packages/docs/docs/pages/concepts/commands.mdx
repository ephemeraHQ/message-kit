...
First think of which command you want the bot to respond to. For example
...
Finally, the user sends the command to the bot.
...
_Note: The [helper](https://github.com/xmtp-labs/botkit/blob/main/packages/botkit/src/lib/helper.ts) function has limitations in parameter extraction. This method is just one way to do it. We could consider AI-driven methods that are more adaptable for various command types. Within this repository, the following is an example of such a parser._
