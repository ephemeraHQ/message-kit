...
```tsx
{
  ...
  content: {}, // The content of the message depending on the content type
  senderAddress: string, // The address of the sender
  sent: Date, // The timestamp of the message
  typeId: string; // The content type of the message (text, reply or reaction)
}
```

To learn more about the content types, see [content types](/concepts/content-types/text)
