...
│   └── prompt.ts         # Prompt for your agent. // [!code hl] // [!code focus]
...
run(async (context: XMTPContext) => {
  const {
    message: {
      id,
      content: { text, skill, params, attachment, reply, reference, reaction },
      sender,
      sent,
      typeId,
    },
...
> This is used in the [Cron](/middleware/cron) middleware to send updates to the users.
