...
const isCreated = await walletService.createWallet(identifier);
...
// Check balance
const balance = await walletService.checkBalance(identifier);

// Transfer between wallets
await walletService.transfer(fromWallet, toAddress, amount);

// Swap assets
await walletService.swap(address, fromAssetId, toAssetId, amount);
```
...
- Only people with the groupId can access the wallets
