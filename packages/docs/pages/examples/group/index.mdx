...
- [AI](/examples/group/agents): Reply with GPT when called by `/agent`.
- [Tipping](/examples/group/tipping): Tip when a user reacts with the `🎩` emoji, replies `10 $degen` and `/tip`.
- [Games](/examples/group/games): Handle frame games like `/game slot`, `/game wordle`
- [Transactions](/examples/group/transactions): Handle commands like `/send`, `/swap`, `/show`.
- [Loyalty](/examples/group/loyalty): Handle loyalty commands like `/points`, `/leaderboard`.
