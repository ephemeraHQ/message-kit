...
- **Bots can’t read messages in chats**
  Bots can’t read _messages_ in chat and can only read _skills_ in chat, such as `/help` or `@bot`. This ensures that bots can act on skills sent by human members while preserving the privacy of everyone in the chat.
- **Bots can’t send unprompted messages**
  Bots can’t send _unprompted_ messages and can only send messages in _response to skills_ sent by human members or if a group chat admin grants them permission to send messages freely. By ensuring that bots only send messages as requested, we keep chats focused and minimize unnecessary noise.
