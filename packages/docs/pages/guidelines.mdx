...
- **Bots can’t read messages in chats**
  Bots can’t read _messages_ in chat and can only read _commands_ in chat, such as `/help` or `@bot`. This ensures that bots can act on commands sent by human members while preserving the privacy of everyone in the chat.
