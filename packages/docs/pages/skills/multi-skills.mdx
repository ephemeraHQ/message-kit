...
The `Multi Skills` processing happens in the `processMultilineResponse` function in the [GPT middleware](/middlewares/gpt.mdx), which takes the AI's response, splits it into individual messages, and handles each one appropriately:
