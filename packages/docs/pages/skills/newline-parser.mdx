...
The bot will reply in full with text and different commands it can execute
...
The `Newline Parser` processing happens in the `processMultilineResponse` function in the [GPT middleware](/middlewares/gpt.mdx), which takes the AI's response, splits it into individual messages, and handles each one appropriately:
