...
As we seen in previous sections, the agent can handle complex interactions by managing multiple steps in sequence, maintaining context, and waiting for user responses.

For full reasoning we need to combine the following skills:

- [Fine Tuning](/skills/fine-tuning): agent can handle fine tuned responses.
- [Prompting](/skills/prompting): agent can handle multiple responses from the AI.
