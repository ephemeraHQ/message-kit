...
export class WalletService {
  ...
  static async transfer(
    ...
    try {
      ...
      const token = balances.find(b => b.token.symbol.toUpperCase() === tokenSymbol.toUpperCase());
      
      if (!token) {
        throw new Error(`Token ${tokenSymbol} not found in wallet. Available tokens: ${balances.map(b => b.token.symbol).join(', ')}`);
      }

      // Check if balance is sufficient
      if (parseFloat(token.amount) < parseFloat(amount)) {
        throw new Error(`Insufficient ${tokenSymbol} balance. Available: ${token.amount}`);
      }
  ...
  static async getTokenBalance(walletData: WalletData, tokenSymbol: string): Promise<TokenBalance | null> {
    const balances = await this.checkBalance(walletData);
    return balances.find(b => b.token.symbol.toUpperCase() === tokenSymbol.toUpperCase()) || null;
  }

  static async checkTransactionStatus(transactionId: string): Promise<TransactionStatus> {
    try {
